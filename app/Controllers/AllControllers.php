...
class SchoolSettingsController extends Controller {
    ...
    public function update(): void {
        ...
        $websiteEnabled = !empty($_POST['website_enabled']) ? 1 : 0;

        $this->db->execute("UPDATE tenants SET name=?,email=?,phone=?,address=?,country=?,timezone=?,academic_year=?,currency=?,domain=?,primary_color=?,secondary_color=?,accent_color=?,logo=?,student_login_mode=?,parent_login_mode=?,restrict_parent_arrears=?,website_enabled=? WHERE id=?",
            [
                ...
                $logo, $studentLoginMode, $parentLoginMode, $restrictParentArrears, $websiteEnabled,
                $this->tid
            ]);
