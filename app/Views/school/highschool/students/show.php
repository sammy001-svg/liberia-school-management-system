...
<div class="profile-layout">
  ...
  <div class="profile-stack">
    ...
    <div class="card">
      ...
      <div class="table-wrapper"><table>
        <thead><tr><th>Date</th><th>Category</th><th>Severity</th><th>Notes</th><th>Reported By</th><?php if(!empty($canManageDiscipline)): ?><th></th><?php endif; ?></tr></thead>
        <tbody>
          ...
          <tr>
            ...
            <td>
              <form method="POST" action="<?= $cfg['url'] ?>/school/discipline/<?= $d['id'] ?>/delete" data-confirm="Remove this disciplinary record?" data-confirm-title="Remove Record" data-confirm-label="Remove">
                <input type="hidden" name="csrf_token" value="<?= htmlspecialchars($csrf_token) ?>">
                <input type="hidden" name="back_to" value="<?= $cfg['url'] ?>/school/students/<?= $student['id'] ?>">
          ...
          <tr><td colspan="<?= !empty($canManageDiscipline) ? 6 : 5 ?>"><div class="empty-state"><div class="empty-state-icon">🛡️</div><div class="empty-state-text">No disciplinary records for this student.</div></div></td></tr>
          <?php endif; ?>
        </tbody>
      </table></div>
    </div>
