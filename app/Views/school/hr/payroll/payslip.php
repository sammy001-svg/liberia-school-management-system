...
<title>Payslip — <?= htmlspecialchars($record['name']) ?> — <?= date('F Y', mktime(0,0,0,$record['month'],1,$record['year'])) ?></title>
<style>
  * { box-sizing:border-box; margin:0; padding:0; }
  ...
  .sheet { width:8.27in; min-height:6in; background:#fff; box-shadow:0 8px 24px rgba(0,0,0,0.18); padding:0.5in 0.6in; }
  ...
  .letterhead .doctitle .period { font-size:11px; color:#6b7280; margin-top:6px; }

  .staffbar { display:grid; grid-template-columns:repeat(3,1fr); gap:10px; background:#f9fafb; border:1px solid #e5e7eb; border-radius:10px; padding:12px 16px; margin-bottom:20px; }
  .staffbar .lbl { font-size:9px; text-transform:uppercase; letter-spacing:0.05em; color:#9ca3af; font-weight:700; }
  .staffbar .val { font-size:12.5px; color:#111827; font-weight:600; margin-top:2px; }
...
<div class="sheet">
  ...
  <div class="letterhead">
    ...
    <div class="doctitle">
      <div class="tag">PAYSLIP</div>
      <div class="period"><?= date('F Y', mktime(0,0,0,$record['month'],1,$record['year'])) ?></div>
    </div>
  </div>

  <div class="staffbar">
    <div><span class="lbl">Staff Name</span><span class="val"><?= htmlspecialchars($record['name']) ?></span></div>
    <div><span class="lbl">Role</span><span class="val"><?= htmlspecialchars($record['role_name']) ?></span></div>
    <div><span class="lbl">Status</span><span class="val"><span class="status-tag status-<?= $record['status']==='paid'?'paid':'draft' ?>"><?= strtoupper($record['status']) ?></span></span></div>
  </div>

  <table class="pay">
    <thead><tr><th>Description</th><th style="text-align:right;">Amount (<?= htmlspecialchars($currency) ?>)</th></tr></thead>
    <tbody>
      <tr><td>Basic Salary</td><td style="text-align:right;"><?= number_format($record['basic_salary'],2) ?></td></tr>
      <tr><td>Allowances</td><td style="text-align:right;color:#059669;">+<?= number_format($record['allowances'],2) ?></td></tr>
      <tr><td>Deductions</td><td style="text-align:right;color:#DC2626;">-<?= number_format($record['deductions'],2) ?></td></tr>
    </tbody>
    <tfoot><tr><td>Net Salary</td><td style="text-align:right;"><?= htmlspecialchars($currency) ?><?= number_format($record['net_salary'],2) ?></td></tr></tfoot>
  </table>

  <div class="signatures">
    <div><div class="line">Accountant / HR</div></div>
    <div><div class="line">Staff Signature</div></div>
  </div>

  <div class="doc-footer">Generated on <?= date('F d, Y \a\t H:i') ?> · This is a computer-generated document.<?= $record['paid_at'] ? ' · Paid on '.date('M d, Y', strtotime($record['paid_at'])) : '' ?></div>
